...
if live:
    camera = PiCamera()
#    camera.shutter_speed = 4717 # 9, 2354, 4717, 7080, 9443, 11806, 14169, 16532, 18895, 21258  Assign shutter speed to non-zero first
...
low_thresh = 220  # Lower threshold for the thresholding function
...
#        pass
    ...
    print(seconds)
    start = time.time()
    im = frame.array  # get current frame
#    im = cv.remap(im, mapx, mapy,cv.INTER_LINEAR)  # Undistort the image
    ...
    for contour in contours:
        ...
        if min_radius < radius < max_radius:
            ...
            # cv.circle(im, center, int(radius), (0, 0, 255), -1)
